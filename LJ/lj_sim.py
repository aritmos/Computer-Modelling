...
def force_LJ(mic_pair_separation: np.array) -> np.array:
    ...
    if r > 3.5:  # Implement cutoff radius
        ...
        )  # -ve due to using r_ij in the formula
        return F
...
def main():
    ...
    if LOG_MSD:
        print(
            ...
            f'{"" if t_multiple == 1 else t_multiple} '
            f'step{"s" if t_multiple > 1 else ""}'
        )
    ...
    if LOG_RDF:
        print(
            ...
            f'{"" if t_multiple == 1 else t_multiple} '
            f'step{"s" if t_multiple > 1 else ""}, with a resolution of {res})'
        )
        histogram = np.zeros(res)
        max_distance = cell_length * (
            np.sqrt(3) / 2 + 0.001
        )  # extra distance to avoid floating point
        dr = max_distance / res
    ...
    for t_step in tqdm(range(total_steps)):
        ...
        if LOG_MSD and (t_step % t_multiple) == 0:
            particle_positions = np.array(
                [particle.pos for particle in p3d_list])
            separations = particle_positions - initial_positions
            mic_separation_list = np.array(
                [pbc.minimum_image(i, cell_length) for i in separations]
            )
